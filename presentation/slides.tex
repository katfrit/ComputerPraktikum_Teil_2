...
\begin{frame}{Ball Tree: Konzept}
    \begin{tikzpicture}[overlay, scale=1.2, yshift=0.0cm, xshift=4.0cm]
        ...
        \begin{scope}[xshift=3.0cm, yshift=-0.5cm]
            \node<2-> at (-0.4,0.0)  {\large \textbf{\red{An jedem Knoten}:}};
            \node<3-> at (0.3,-0.5)  {1. Mittelpunkt, Radius berechnen};
            \node<4-> at (0.25,-1.0) {2. Zwei entfernte Punkte wählen};
            \node<5-> at (0.2,-1.5)  {3. Daten aufteilen nach Distanz};
            \node<6-> at (0.24,-2.0)  {4. Repeat until leaf size reached};
...
\begin{frame}[fragile]{Ball Tree: Rekursiv}
...
\begin{frame}[fragile]{Ball Tree: Rekursive Laufzeit}
    \begin{enumerate}
        \item Verwenden von \textbf{\red{built-in}} Funktionen (C-optimized). \onslide<4->{{\Huge \dgreen{\checkmark}} }
...
\red{def} \purple{vector_sum}(vectors):
    \red{return} [\purple{sum}(components) \blue{for} components \blue{in} \purple{zip}(\blue{*}vectors)]
        ...
        \item<3-> \red{\textbf{Lokales Speichern}} von oft verwendeten Variablen. \onslide<6->{{\Large \red{\text{\sffamily X}}} }
    \end{enumerate}
    \vspace{0.7cm}
    \LARGE \onslide<7->{\textbf{Rekursive Methode nicht optimal \red{!!!}}}
\end{frame}
...
% ----------------------------------------------------------------------------------------
    ...
    \begin{frame}{Ball Tree: iterative Methode}
        \centering 
        \begin{tikzpicture}
            \node (node01) [startstop] {\textbf{Stack initialisieren:} Solange Stack nicht leer - nächste Node heraus holen};
            \node (node02) [startstop, below of=node01, yshift=-1.5cm, xshift=-2.75cm] {Wenn Datenmenge klein genug ist: Leaf-Node erstellen};
            \node (node03) [startstop, below of=node01, yshift=-1.5cm, xshift=2.75cm] {Daten splitten und beide Kinder zum Stack hinzufügen\\
            LIFO: erst rechts, dann links};
            \draw [arrow] (node01) -- node[midway, below, xshift=0.25cm] {} (node02);
            \draw [arrow] (node01) -- node[midway, below, xshift=0.25cm] {} (node03);
        \end{tikzpicture}
    \end{frame}

    \begin{frame}{Speed Ups}
        \begin{enumerate}
            \item built-in Funktionen $\Rightarrow$ C-optimiert
            \item Best-First statt Depth-First $\Rightarrow$ intelligente Baum-Suche
            \item Binomische Formel $\Rightarrow$ weniger Rechenoperationen
            \item Norm-Vorberechnung $\Rightarrow$ Caching-Strategie
            \item Global Pruning $\Rightarrow$ Frühe Terminierung
        \end{enumerate}
    \end{frame}


    \begin{frame}{Speed Ups: Best-First statt Depth-First}
        \begin{itemize}
            \item \textbf{Alt: Depth-First}
            \begin{itemize}
            \item Stack-LIFO
            \item Näheres Kind zuerst (lokal)
            \item keine globale Priorisierung
        \end{itemize}

            \item \textbf{Neu: Best-First}
            \begin{itemize}
            \item Queue statt Stack
            \item Sortieren die Queue bei jedem Loop nach der Distanz
            \item Mit \texttt{reverse=True} und \texttt{.pop()} holen wir Node mit kleinster Distanz zum Query-Punkt $\Rightarrow$ besuchen vielversprechendste Node als nächste
            \item Beim Hinzufügen von Kindern speichern wir auch deren Distanz: Tupel \texttt{d-child}, \texttt{child-idx} erlaubt Sortierung
        \end{itemize}
        $\Rightarrow$ Wir besuchen deutliche weniger Nodes, weil weit entfernte Bereiche früher gepruned werden..
        \end{itemize}
    \end{frame}

    \begin{frame}[fragile]{Speed Ups: Best-First statt Depth-First}
    \begin{figure}
    ...
    \begin{frame}[fragile]{Speed Ups: Binomische Formel}
        \begin{columns}
        \column{0.5\textwidth}
        \textbf{Alt: Naive Distanz}
        \begin{lstlisting}[language=Python, basicstyle=\tiny\ttfamily]
        def query(target, k):
            # Fuer jeden Punkt:
            d_sq = sum(
        ...
        Kosten bei $d=15$:
        \begin{itemize}
            \item 15 Subtraktionen
            \item 15 Quadrierungen
            \item 14 Additionen
        \end{itemize}
        \textbf{44 Operationen}

        
        \column{0.5\textwidth}    
        \textbf{Neu: Binomische Formel}
        \begin{lstlisting}[language=Python, basicstyle=\tiny\ttfamily]
        ...
        \end{lstlisting}
        Kosten bei $d=15$:\\
        \textbf{33 Operationen}
        \end{columns}
    \end{frame}

\begin{frame}{Fehlerreduktion: Plots}
        \begin{multicols}{2}
        \begin{figure}
            \centering
            \includegraphics[width=\linewidth]{team-2-bananas-1-2d-old.train.png}
            \caption{Alt: Sequentielles Plotten}
        \end{figure}

        \begin{figure}
            \centering
            \includegraphics[width=\linewidth]{team-2-bananas-1-2d.train.png}
            \caption{Neu: Zufälliges Plotten}
        \end{figure}
        \end{multicols}

        \begin{multicols}{2}
        \begin{itemize}
            \item Klassen nacheinander geplottet
            \item Klasse -1 überdeckt +1
            \item \texttt{random.shuffle()} vor Plot
            \item Tatsächliche Überlappung der Klassen $\Rightarrow$ \glqq wahre\grqq\, Struktur
        \end{itemize}
    ...
    \begin{frame}{Fehlerreduktion: \texttt{check accuracy.py}}
        \textbf{Zweck:} Vergleiche \texttt{test.csv} mit \texttt{result.csv} und berechne Genauigkeit\\
        \textbf{Nutzen:} Direkte Datenvalidierung\\
        \textrm{}\\
        \centering 
        \begin{tikzpicture}
            \node (node01) [startstop] {\textbf{Lade} \texttt{true-labels} \textbf{und} \texttt{pred-labels}};
            \node (node02) [startstop, below of=node01, yshift=-1.cm] {\textbf{Vergleiche\\elementenweise}};
            \node (node03) [startstop, below of=node02, yshift=-1.cm] {\textbf{Genauigkeit berechnen}};
            \draw [arrow] (node01) -- node[midway, below, xshift=0.25cm] {} (node02);
            \draw [arrow] (node02) -- node[midway, below, xshift=0.25cm] {} (node03);
        \end{tikzpicture}

    \end{frame}


    \begin{frame}{Lernerfolge: Technische Skills}   
    \begin{enumerate}
        ...
        \begin{itemize}
                \item Iterative Entwicklung: Rekursiv $\to$ Iterativ
                \item Testing \& Debugging
                \item Dokumentation
            \end{itemize}

        \item \textbf{Datenstrukturen \& Algorithmen}
        \begin{itemize}
                \item BallTree
                \item Komplexität
            \end{itemize}

        \item \textbf{Performance-Optimierung:}
        \begin{itemize}
                \item Profiling: Bottleneck-Identifikation
                \item Algorithmen: 
                \item Mathematik: Binomische Formel
            \end{itemize}
    \end{enumerate}
    ...
    \begin{frame}{Lernerfolge: Theoretisches Verständnis}
        \begin{multicols}{2}
            \begin{figure}
                \centering
                \includegraphics[width=\linewidth]{team-2-bananas-1-2d.log.png}
                \caption{Fehlerkurve \texttt{bananas-1-2d}}
            \end{figure}


            \begin{figure}
                \centering
                \includegraphics[width=\linewidth]{team-2-magic_gamma_telescope.small.log.png}
                \caption{Fehlerkurve \texttt{magic gamma telescope.small}}
            \end{figure}
        \end{multicols}
    \end{frame}
